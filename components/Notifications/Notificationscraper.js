...
const NotificationScraper = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(false);
        var sortedData = notificationdata.sort(
    ...
    <div key="notification">
      {isLoading ? (
        ...
        <div>
          <div className=" bg-gradient-to-br from-indigo-50 via-white to-cyan-100 pt-[75px]    hidden md:block font-interer">
            <div className="max-w-[800px] justify-center mx-auto">
              ...
              <div className="w-[75%] inline-block ">
                <input
                  id="searchkey"
                  placeholder="I Year II semester"
                  onChange={handleSearch}
                  className="border-[2px] border-solid border-[#bfbcbc] pl-[30px] w-[100%] h-[35px] ml-[45px]"
                />
              </div>
              <div className="inline-block float-right mr-[50px]  z-5 ">
                <select
                  className="h-[35px] w-[108px] font-[16px] border-[2px] border-solid  border-[#bfbcbc]"
          ...
          <h1 className="text-3xl font-semibold text-center pt-[75px] mb-8 font-interer md:hidden">
            Notifications
          </h1>
          {results.map((result, index) => (
            <div
              key={index}
              className="bg-[white] text-left p-[20px] mb-[3px] pb-[5px] md:hidden"
            >
