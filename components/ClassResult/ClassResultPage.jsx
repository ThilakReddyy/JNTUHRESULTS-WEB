...
const AcademicReportPage = () => {
  ...
  const submit = async () => {
    if (
      form["hallticketNo"].length != 10 ||
      form["hallticketNo"].substring(2, 4) !== form["collegeOption"]
    ) {
      setWarning("The hallticket Number and the college does not match");
    }
    console.log("here");
    ...
    if (currentHour >= 7) {
      ...
      setWarning("Results are only available from 12AM to 7 AM.");
      return "";
    }
    ...
    try {
      ...
      for (let reg = 0; reg < regularorLateral.length; reg++) {
        ...
        if (reg === 1) {
          if (
            form["semesterOption"] === "1-1" ||
            form["semesterOption"] === "1-2"
          ) {
            break;
          }
        ...
        const hallticket =
          regulation +
          form["collegeOption"] +
          regularorLateral[reg] +
          form["degreeOption"] +
          form["branchOption"];

        // Fetch data for different roll numbers
        const url =
          "https://jntuhresults.up.railway.app/api/classresult?semester=" +
          form["semesterOption"] +
          "&htnos=";
        for (let i = 0; i < roll_last_2.length; i++) {
          ...
          var storedData = localStorage.getItem(
            roll_number + form["semesterOption"],
          );
          if (storedData !== null) {
  ...
  return (
    <div className="">
      ...
      <div
        className={`${!reportForm && !loading ? "block" : "hidden"} pt-[55px]`}
      >
