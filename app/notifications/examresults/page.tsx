...
const NotificationExamResults = () => {
  ...
  const storage: string = localStorage.getItem(htno + encodeURIComponent(query)) || "";
          ...
          <table style={{ borderCollapse: "collapse", borderSpacing: 0, border: "none", width: "100%" }}>
            <thead>
              <tr className="bg-gray-50 dark:bg-white/5 border-b border-gray-200 dark:border-white/10">
                {["Code", "Subject Name", "Int.", "Ext.", "Total", "Grade", "Cr."].map((h) => (
                  <th
                    key={h}
            ...
            <tbody>
              {results.map((result: any, i: number) => {
                ...
                const rowBg = i % 2 === 0 ? "bg-white dark:bg-transparent" : "bg-gray-50/80 dark:bg-white/[0.03]";
                return (
                  <>
                    ...
                    <tr key={`n-${i}`} className={`md:hidden border-b border-gray-100 dark:border-white/5 ${rowBg}`}>
                      <td colSpan={6} className="px-2.5 pb-2 pt-0 text-[11px] text-gray-500 dark:text-gray-400 italic leading-snug" style={{ border: "none" }}>
                        {result["subject_name"]}
                      </td>
                    </tr>
                  </>
